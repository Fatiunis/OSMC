...
try {
    ...
    $stmt = $conn->prepare("INSERT INTO medicamentos 
        (codigo, nombre, categorias, principio_activo, descripcion, fotografia_url, concentracion,
         presentacion, unidades, marca, requiere_receta, en_stock, precio)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");

    $stmt->bind_param("sssssssssisid",
        $codigo, $nombre, $categorias, $principio_activo, $descripcion, $foto,
        $concentracion, $presentacion, $unidades, $marca, $requiere_receta, $en_stock, $precio
    );

    if ($stmt->execute()) {
        echo json_encode(["success" => true, "message" => "✅ Medicamento registrado correctamente."]);
    } else {
        throw new Exception("❌ Error al insertar el medicamento: " . $stmt->error);
    }
